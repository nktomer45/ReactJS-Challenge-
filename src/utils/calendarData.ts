...
/**
 * Calendar data showing week to month mappings
 */
...
/**
 * Calendar data for the fiscal year
 */
...
/**
 * Get calendar item by week code
 */
export function getCalendarItemByWeek(weekCode: string): CalendarItem | undefined {
  return calendarData.find(item => item.week === weekCode);
}

/**
 * Generate weeks using the calendar data
 */
...
/**
 * Get all unique months from calendar data
 */
export function getUniqueMonths(): {month: string, label: string}[] {
  const uniqueMonths = new Map<string, string>();
  
  calendarData.forEach(item => {
    if (!uniqueMonths.has(item.month)) {
      uniqueMonths.set(item.month, item.monthLabel);
    }
  });
  
  return Array.from(uniqueMonths.entries()).map(([month, label]) => ({
    month,
    label
  }));
}
